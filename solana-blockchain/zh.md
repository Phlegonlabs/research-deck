...
flowchart TD
    A["客户端发送交易"]
    B["Gulf Stream：转发给即将到来的 leader（无内存池）"]
    C["TPU 流水线：获取 → 验证签名(GPU) → 执行(CPU) → 写入(磁盘)"]
    D["Sealevel：基于声明的账户访问进行并行执行"]
    E["Cloudbreak：并发读写账户数据库"]
    F["PoH：将结果加时间戳到哈希链中"]
    G["Turbine：通过树形层级传播区块 shred"]
    H["Tower BFT：验证者使用 PoH 时钟投票（O(n) 消息）"]
    I["归档器：使用复制证明存储历史数据"]
...
flowchart TD
    A["用户上传图片 + 选择名称/代码"]
    B["代币在联合曲线上创建"]
    C["立即在 pump.fun 上开始交易"]
    D["'毕业' → 在 Raydium DEX 上市"]
    E["Pump.fun 赚取：1%交换费 + 1.5 SOL 毕业费"]

    A -- "花费 < $2" --> B --> C -- "如果市值达到 $90K" --> D --> E
```
