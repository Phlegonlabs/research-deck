...
flowchart TD
    subgraph UL["用户层"]
        RU["普通用户"] --> RPC["RPC 节点"] --> REL["Relayer"] --> VAL["验证者\n(Jito-Solana)"]
        SR["搜索者"] --> BEn["区块引擎"] --> VAL
    end
...
sequenceDiagram
    ...
    participant BE as 区块引擎
    participant V as 验证者

    S->>BE: 提交 Bundle (最多5笔 + 小费)
    BE->>BE: 基本检查
    BE->>BE: 模拟所有交易
    BE->>BE: 比较小费
    BE->>BE: 按状态锁分组
...
flowchart TD
    ...
    subgraph SK["质押者层"]
        SL["用户存入 SOL/JitoSOL/JTO → 收到 VRT\nVRT 是流动的 → 可在 DeFi 中使用"]
    end
...
```mermaid
flowchart TD
    A["更多验证者运行 Jito-Solana"] --> B["更多搜索者提交 bundle\n（更大市场）"]
    B --> C["更高小费收入"]
    C --> D["更高 JitoSOL 收益\n（MEV 小费与质押者共享）"]
    D --> E["更多 SOL 质押为 JitoSOL"]
    E --> F["更多委托给 Jito 验证者"]
    F --> G["更多验证者被激励\n运行 Jito-Solana"]
    G --> A
```
